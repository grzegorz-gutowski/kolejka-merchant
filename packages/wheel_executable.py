...
#    thrift_dir = Path(work_dir) / 'thrift'
#    thrift_dir.mkdir()
#    thrift_tar = list(pip_dir.glob('thriftpy2*.tar*'))[0]
#    subprocess.run(['tar', 'ax', '--wildcards', '-f', thrift_tar, '*/thriftpy2'], cwd=thrift_dir, check=True)
#    subdir = list(thrift_dir.glob('*'))[0]
    with (extracted_dir / 'thriftpy2' / '_compat.py').open('a') as compat_file:
        compat_file.write('CYTHON = False')
#    subprocess.run(['rsync', '-a', str(subdir)+'/', str(extracted_dir)], cwd=work_dir, check=True)
